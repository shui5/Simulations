function [outA,outB,outC,outD] = sim_signals(MRS_opt)
%run the loop for every metabolite
%sim_signals copied from muhammadsaleh87 on 2/2/21 by RE
%First step, try to add in simulation of PRESS
...
for ii = 1:length(MRS_opt)
    ...
    if strcmp(MRS_opt(1).seq, 'HERC')
        ...
        elseif strcmp(MRS_opt(1).seq, 'MEGA')
            editOnFreq1 = MRS_opt(ii).editOnFreq1;
            editOnFreq2 = MRS_opt(ii).editOnFreq2;
            
        else strcmp(MRS_opt(1).seq, 'PRESS') 
            %no editing pulses needed. 
            %Integrated into the same file 0202021 RAEE
    end
        
    if strcmp(MRS_opt(1).seq, 'PRESS')                  %Integrated into the same file 0202021 RAEE
    taus = [MRS_opt(ii).TE1/2,...                                  %middleEXC pulse to middleREFOC1
         TE/2 ,...                                                  %middleREFOC1 to middle REFOC2
        (TE/2-MRS_opt(ii).TE1/2)];... %middle REFOC2 to the start of readout
    else
    % ********************PARAMETERS**********************************
    ...
    if strcmp(MRS_opt(1).seq, 'PRESS')                  %Integrated into the same file 0202021 RAEE
       delays    = MRS_opt(ii).delays; %zeros(size(taus));
        ...
        delays(2) = taus(2)-MRS_opt(ii).refTp+MRS_opt(ii).refTp/2;
        delays(3) = taus(3)-MRS_opt(ii).refTp/2;
    else
        delays    = MRS_opt(ii).delays; %zeros(size(taus));
        delays(1) = taus(1)-(MRS_opt(ii).refTp/2);
    ...
    %% Start the sequence (initially the same for PRESS and edited PRESS)
    ...
    elseif strcmp(MRS_opt(ii).seq, 'HERM')
        MRS_opt(ii).editRF11   = dual_lobe_split_fn(MRS_opt(ii).editRF1,polyval(MRS_opt(ii).split_fit,dl_split1),polyval(MRS_opt(ii).B1power_fit,dl_split1));
        MRS_opt(ii).editRFonA  = rf_freqshift(MRS_opt(ii).editRF11,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq1)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %mean([editOnFreq2,editOnFreq3])
        MRS_opt(ii).QoutONA    = calc_shapedRF_propagator_edit(MRS_opt(ii).H,MRS_opt(ii).editRFonA,MRS_opt(ii).editTp,MRS_opt(ii).edit_flipAngle,0);
        ...
        MRS_opt(ii).editRFonD  = rf_freqshift(MRS_opt(ii).editRF1,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq4)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %editOnFreq3
        MRS_opt(ii).QoutOND    = calc_shapedRF_propagator_edit(MRS_opt(ii).H,MRS_opt(ii).editRFonD,MRS_opt(ii).editTp,MRS_opt(ii).edit_flipAngle,0);
    else
        ...
        MRS_opt(ii).editRFonB  = rf_freqshift(MRS_opt(ii).editRF1,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq2)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %editOnFreq1
        MRS_opt(ii).QoutONB    = calc_shapedRF_propagator_edit(MRS_opt(ii).H,MRS_opt(ii).editRFonB,MRS_opt(ii).editTp,MRS_opt(ii).edit_flipAngle,0);
    end
    ...
    if strcmp(MRS_opt(ii).seq, 'PRESS')
        [d_A,~,~,~]   = sim_megapress_shaped_ultrafast_edit1(MRS_opt(ii));
    else
    ...
    for Y=1:length(MRS_opt(ii).y)  %Use this if you do have the MATLAB parallel processing toolbox
        ...
        if strcmp(MRS_opt(ii).seq, 'PRESS')
            [outA_temp{Y},~,~,~]                       = sim_megapress_shaped_ultrafast_Ref2(MRS_opt(ii),MRS_opt(ii).Qrefoc{Y},d_A);
        else
            if ~strcmp(MRS_opt(ii).seq, 'MEGA')
    ...
    if strcmp(MRS_opt(ii).seq, 'PRESS')
        [outA,~,~,~]       = sim_megapress_shaped_ultrafast_readout(MRS_opt(ii),d_Ay);
    else
        if ~strcmp(MRS_opt(ii).seq, 'MEGA')
...
if strcmp(MRS_opt.seq, 'PRESS')
    d1       = sim_evolve(d1,MRS_opt.H,MRS_opt.delays(3)/1000);                            %Evolve by delays(5)
end
