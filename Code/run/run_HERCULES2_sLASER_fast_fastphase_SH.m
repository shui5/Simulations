...
metabolites = {'H2O'};
[outA,outB,outC,outD] = sim_HERCULES2_sLASER_fast_fastphase_SH(metabolites);
toc
%time_elapsed=toc;
% outSum.ppm = outA.ppm;
...
x_lim = [1 5]
y_lim = [-0.2 0.4]
figure(1),subplot(4,1,1),plot(outA.ppm,outA.specs,'r','linewidth',2),set(gca,'xdir','reverse'),ylim(y_lim),xlim(x_lim), xlabel('ppm'),title('A: 4.58ppm')
figure(1),subplot(4,1,2),plot(outB.ppm,outB.specs,'b','linewidth',2),set(gca,'xdir','reverse'),ylim(y_lim),xlim(x_lim), xlabel('ppm'),title('B: 4.04ppm')
figure(1),subplot(4,1,3),plot(outC.ppm,outC.specs,'g','linewidth',2),set(gca,'xdir','reverse'),ylim(y_lim),xlim(x_lim), xlabel('ppm'),title('C: 4.58ppm 1.90ppm')
figure(1),subplot(4,1,4),plot(outD.ppm,outD.specs,'k','linewidth',2),set(gca,'xdir','reverse'),ylim(y_lim),xlim(x_lim), xlabel('ppm'),title('D: 4.04ppm 1.90ppm')

% plot GABA
figure(2), plot(outA.ppm,outC.specs+outD.specs-outA.specs-outB.specs,'k'),set(gca,'xdir','reverse'),xlim(x_lim),ylim(y_lim), xlabel('ppm'),legend('GABAGlx')
