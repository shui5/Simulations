...
%tic;
function [out] = sim_sLASER_fast_fastphase_TE35(metabolites)
...
for kk=1:numel(metabolites)
    ...
    spinSys     = metabolite;
    out_name    = ['Philips_sLASER_TE35_41pts_' spinSys '.mat']; 
    
    % INPUTS:
n=8192; %= number of points in fid/spectrum
sw=4000; %= desired spectral width in [Hz]
Bfield=3;%7; %= main magnetic field strength in [T]
lw=1; %= linewidth in [Hz]
...
nX=101;%16; %Number of grid points to simulate in the x-direction
nY=101;%16; %Number of grid points to simulate in the y-direction
te=35;%135;         %sLASER total echo time [ms]
