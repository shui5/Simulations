...
%
% DESCRIPTION:
% This script simulates a HERCULES_1 (equivalent to HERMES 3) experiment using Philips product sequence with fully shaped editing
...
function [outA,outB,outC,outD] = sim_HERCULES2_PRESS_fast_fastphase_SH(metabolites)

addpath(genpath('/Users/steve/Documents/MATLAB/FID-A'));
% addpath(genpath('/home/shui5/Simulation/FID-A')); %This is for KKI - SH 07252019
...
for kk=1:numel(metabolites)
    ...
    spinSys     = metabolite;
    out_name    = ['Philips_HERC2_PRESS_fast_fastphase_' spinSys '.mat']; 
    ...
    editWaveform2       = 'am_sg_150_100_200pts.pta';   % name of 2nd single editing pulse waveform. [4.04ppm]
    editWaveform3       = 'dl_Philips_4_58_1_90.pta'; % name of 1st dual editing pulse waveform. [4.58ppm 1.90ppm]
    editWaveform4       = 'dl_Philips_4_04_1_90.pta'; % name of 2nd dual editing pulse waveform. [4.04ppm 1.90ppm]
    
    % Define frequency parameters for editing targets
    flipAngle   = 180;
    centreFreq  = 3.0;              % Center frequency of MR spectrum [ppm]
    editOnFreq1 = 4.58;             % Center frequency of 1st HERCULES_2 experiment [ppm]
    editOnFreq2 = 4.04;             % Center frequency of 2nd HERCULES_2 experiment [ppm]
    editOnFreq3 = (4.58+1.9)/2;     % Center frequency of 3rd HERCULES_2 experiment [ppm]
    editOnFreq4 = (4.04+1.9)/2;     % Center frequency of 4th HERCULES_2 experiment [ppm]
    ...
    refTp       = 6.894;            % duration of refocusing pulses [ms], set to zero for hard pulse
...
%     excRF = io_loadRFwaveform(exciteWaveform,'exc',0); %DJL added %Come back later - SH 07252019
    ...
    %       refRF.tw1 = 9.932;
        case 'gtst1203_sp.pta'
            ...
            refRF.tbw = 1.357 * 6.894; %BW99 (kHz) * dur (ms)
...
%     editRF4.tw1 = editRF1.tw1*2;
    editRF_A          = io_loadRFwaveform(editWaveform1,'inv',0);
    editRF_A.tw1      = 0.93;
    editRF_B          = io_loadRFwaveform(editWaveform2,'inv',0);
    editRF_B.tw1      = 1.4423;
    editRF_C          = io_loadRFwaveform(editWaveform3,'inv',0);
    editRF_C.tw1      = 1.8107;
    editRF_D          = io_loadRFwaveform_SH(editWaveform4,'inv',0);
    temp.waveform = flip(editRF_D.waveform,1);
    editRF_D.waveform = temp.waveform;
    editRF_D.tw1      = 1.9794;
    
    % Construct the editing pulses from the waveforms and defined
    % frequencies
    editRFonA=rf_freqshift(editRF_A,editTp1,(centreFreq-editOnFreq1)*Bfield*gamma/1e6); %4.58
    editRFonB=rf_freqshift(editRF_B,editTp2,(centreFreq-editOnFreq2)*Bfield*gamma/1e6); %4.04
    editRFonC=rf_freqshift(editRF_C,editTp3,(centreFreq-editOnFreq3)*Bfield*gamma/1e6); %(4.58 + 1.90)/2
    editRFonD=rf_freqshift(editRF_D,editTp4,(centreFreq-editOnFreq4)*Bfield*gamma/1e6); %(4.04 + 1.90)/2
...
%     excRF = rf_resample(excRF,100);
    ...
    d_C_temp=cell(length(x));
    d_D_temp=cell(length(x));
...
%     sum_D=sum_A;
   ...
   parfor X=1:length(x)  %Use this if you do have the MATLAB parallel processing toolbox
       ...
       %                 d_A_temp{X}{EP1}{RP1}=sim_megapress_shaped_fastRef1(Bfield,taus,sys,editRFonA,editTp,editPhCyc1(EP1),...
       %                     refRF,refTp,Gx,x(X),refPhCyc1(RP1));
       %                 d_B_temp{X}{EP1}{RP1}=sim_megapress_shaped_fastRef1(Bfield,taus,sys,editRFonB,editTp,editPhCyc1(EP1),...
       %                     refRF,refTp,Gx,x(X),refPhCyc1(RP1));
       %                 d_C_temp{X}{EP1}{RP1}=sim_megapress_shaped_fastRef1(Bfield,taus,sys,editRFonC,editTp,editPhCyc1(EP1),...
       %                     refRF,refTp,Gx,x(X),refPhCyc1(RP1));
       %                 d_D_temp{X}{EP1}{RP1}=sim_megapress_shaped_fastRef1(Bfield,taus,sys,editRFonD,editTp,editPhCyc1(EP1),...
       %                     refRF,refTp,Gx,x(X),refPhCyc1(RP1));
...
d_in=sim_evolve(d,H,delays(1)/1000);                               %Evolve by delays(1)
%d=sim_shapedRF_SH(d,H,refPulse,refTp,flipAngle,90+refPh1,dx,Gx);   %1st shaped 180 degree refocusing pulse
clear d_out1;
%d_in=d;
for kk=1:length(refPh1)
    d_out{kk}=sim_shapedRF(d_in,H,refPulse,refTp,flipAngle,90+refPh1(kk),dx,Gx);   %1st shaped 180 degree refocusing pulse
    % average as we go
end
...
%d=sim_shapedRF_SH(d,H,editPulse,editTp,180,90+editPh1);            %1st shaped editing pulse rotation
clear d_out1;
...
%BEGIN PULSE SEQUENCE************
%d=sim_shapedRF_SH(d,H,refPulse,refTp,flipAngle,90+refPh2,dy,Gy);  %2nd shaped 180 degree refocusing pulse
...
%d=sim_shapedRF_SH(d,H,editPulse,editTp,180,90+editPh2);     %2nd shaped editing pulse rotation
