...
function [outA,outB,outC,outD] = sim_Philips_HERMES_Univ_PRESS_fast_fastphase_SH(metabolites)
...
for kk=1:numel(metabolites)
    ...
    spinSys     = metabolite;
    out_name    = ['Philips_HERMES_Univ_PRESS_41pts_' spinSys '.mat']; 
    
    % General properties of the simulations
    Npts    = 8192;     % number of spectral points
    sw      = 4000;     % spectral width [Hz]
    Bfield  = 3;        % Philips (3T)/Siemens (2.89T) magnetic field strength [Tesla]
