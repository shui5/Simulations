...
addpath /Users/steve/Documents/My_Studies/Simulations/Code/misc/sim_Siemens_Univ_TE80_fast_fastphase.m/
...
metabolites = {'Ref34ppm'};
[outA,outB,outC,outD] = sim_Siemens_HERMES_Univ_TE80_fast_fastphase_SH(metabolites);
toc
%time_elapsed=toc;
% outSum.ppm = outA.ppm;
...
figure(1),subplot(4,1,3),plot(outB.ppm,outC.specs,'b','linewidth',2),set(gca,'xdir','reverse'),xlim(x_lim), xlabel('ppm'),title('B: (4.56+1.9)/2ppm')
figure(1),subplot(4,1,4),plot(outB.ppm,outD.specs,'b','linewidth',2),set(gca,'xdir','reverse'),xlim(x_lim), xlabel('ppm'),title('B: 10ppm')

% plot GABA
figure(2), plot(outA.ppm,outA.specs-outB.specs,'k'),set(gca,'xdir','reverse'),xlim(x_lim),ylim(y_lim), xlabel('ppm'),legend('GABAGlx')
