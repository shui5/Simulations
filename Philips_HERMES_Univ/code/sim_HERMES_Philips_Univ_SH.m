...
function [outA,outB,outC,outD] = sim_HERMES_Philips_Univ_SH(metabolites)
...
for kk=1:numel(metabolites)
    ...
    spinSys     = metabolite;
    out_name    = ['Philips_HERMES_Univ_41pts_' spinSys '.mat']; 
    ...
    editWaveform3       = 'dl_GABA_GSH.pta';        % name of 1st dual editing pulse waveform. [4.56ppm 1.90ppm]
    ...
    excTp       = 7.2;    %2.9008;    % duration of excitation pulse [ms], set to zero for hard pulse
...
%     editRF4.tw1 = editRF1.tw1*2;
    ...
    editRF_B.tw1  = 0.93;
    editRF_C      = io_loadRFwaveform(editWaveform3,'inv',0); % dl_GABA_GSH
    editRF_C.tw1  = 1.8107;
...
%     sum_D=sum_A;
   ...
   parfor X=1:length(x)  %Use this if you do have the MATLAB parallel processing toolbox
    ...
    for X=1:length(x)
        for EP1=1:length(editPhCyc1)
            for RP1=1:length(refPhCyc1)
                d_A{EP1,RP1}=sim_dAdd(d_A{EP1,RP1},d_A_temp{X}{EP1}{RP1});
                d_B{EP1,RP1}=sim_dAdd(d_B{EP1,RP1},d_B_temp{X}{EP1}{RP1});
                d_C{EP1,RP1}=sim_dAdd(d_C{EP1,RP1},d_C_temp{X}{EP1}{RP1});
                d_D{EP1,RP1}=sim_dAdd(d_D{EP1,RP1},d_D_temp{X}{EP1}{RP1});
            end
        end
    end
            ...
            for EP2=1:length(editPhCyc2)
                for RP1=1:length(refPhCyc1)
                    for RP2=1:length(refPhCyc2)
                        ...
                        outA_temp{Y}{EP1}{EP2}{RP1}{RP2}=sim_megapress_shaped_fastRef2(d_A{EP1,RP1},Npts,sw,Bfield,lw,taus,sys,editRFonA,editTp,editPhCyc2(EP2),...
                            refRF,refTp,Gy,y(Y),refPhCyc2(RP2));
                        outB_temp{Y}{EP1}{EP2}{RP1}{RP2}=sim_megapress_shaped_fastRef2(d_B{EP1,RP1},Npts,sw,Bfield,lw,taus,sys,editRFonB,editTp,editPhCyc2(EP2),...
                            refRF,refTp,Gy,y(Y),refPhCyc2(RP2));
                        outC_temp{Y}{EP1}{EP2}{RP1}{RP2}=sim_megapress_shaped_fastRef2(d_C{EP1,RP1},Npts,sw,Bfield,lw,taus,sys,editRFonC,editTp,editPhCyc2(EP2),...
                            refRF,refTp,Gy,y(Y),refPhCyc2(RP2));
                        outD_temp{Y}{EP1}{EP2}{RP1}{RP2}=sim_megapress_shaped_fastRef2(d_D{EP1,RP1},Npts,sw,Bfield,lw,taus,sys,editRFonD,editTp,editPhCyc2(EP2),...
                            refRF,refTp,Gy,y(Y),refPhCyc2(RP2));
    ...
    for Y=1:length(y)
        for EP1=1:length(editPhCyc1)
            for EP2=1:length(editPhCyc2)
                for RP1=1:length(refPhCyc1)
                    for RP2=1:length(refPhCyc2)
                        outA=op_addScans(outA,outA_temp{Y}{EP1}{EP2}{RP1}{RP2},xor(RP1-1,RP2-1));
                        outB=op_addScans(outB,outB_temp{Y}{EP1}{EP2}{RP1}{RP2},xor(RP1-1,RP2-1));
                        outC=op_addScans(outC,outC_temp{Y}{EP1}{EP2}{RP1}{RP2},xor(RP1-1,RP2-1));
                        outD=op_addScans(outD,outD_temp{Y}{EP1}{EP2}{RP1}{RP2},xor(RP1-1,RP2-1));
                    end
                end
            end
        end
