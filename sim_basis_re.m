function sim_basis_re(seq,TE,metab,mega_freq)
%function added 020221 by RAEE as a wrapper to call MGS's sim_signals (with
%edits to add PRESS) to generate a basis set.
...
tic
mega_or_hadam          = seq; %Options: HERC, HERM, MEGA
...
elseif strcmp(mega_or_hadam, 'HERM') %HERMES
    ...
    Nmetab                 = length(metab);   %Only metabolite to be run
elseif strcmp(mega_or_hadam, 'MEGA') %HERMES %MEGA-PRESS
    %metab                  = {'GABA'};
    %TE                     = 80;
    %Nmetab                 = 1;   %Only metabolite to be run
    %ppm_min                = [2.80];
    %ppm_max                = [3.20];
    A                      = mega_freq(1); %single-lobe pulse
    B                      = mega_freq(2); %single-lobe pulse
%     create a struct variable
    MRS_temp.seq           = mega_or_hadam; %MRS_temp.HERC_or_HERM    = HERC_or_HERM;
    MRS_temp.metab         = metab; %{ii};
    MRS_temp.Nmetab        = length(metab); %ii;
    MRS_temp.TE           = TE;
...
%if ~strcmp(mega_or_hadam, 'MEGA')
    for ii = 1:Nmetab
        ...
        MRS_temp.metab           = metab{ii};
        MRS_temp.Nmetab          = ii;
        MRS_temp.TEs             = num2cell(MRS_temp.TE);
        %MRS_temp.ppm_min         = ppm_min(ii); %(ii);
        %MRS_temp.ppm_max         = ppm_max(ii); %(ii);
        if strcmp(mega_or_hadam, 'MEGA')
        MRS_temp.editON          = num2cell([A B]);
        elseif strcmp(mega_or_hadam, 'HERM')|| strcmp(mega_or_hadam, 'HERC')
